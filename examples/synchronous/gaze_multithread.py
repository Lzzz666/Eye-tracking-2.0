...
# 儲存影像和數據的緩衝區
image_buffer = {"left_eye": [], "right_eye": [], "scene": []}
data_buffer_csv = []
...
def save_images_and_data():
    global image_buffer, data_buffer_csv, output_dir, frame_count
    while True:
        if recording:
            if frame_count % 1 == 0:  # 每 1 幀保存一次影像和數據
                timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

                # 儲存影像
                if image_buffer["left_eye"]:
                    mpimg.imsave(os.path.join(output_dir, "left_eye", f"{timestamp}.png"), image_buffer["left_eye"][-1])
                if image_buffer["right_eye"]:
                    mpimg.imsave(os.path.join(output_dir, "right_eye", f"{timestamp}.png"), image_buffer["right_eye"][-1])
                if image_buffer["scene"]:
                    mpimg.imsave(os.path.join(output_dir, "scene", f"{timestamp}.png"), image_buffer["scene"][-1])

                # 儲存數據
                with open(os.path.join(output_dir, "gaze_data.csv"), mode='a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerows(data_buffer_csv)
                data_buffer_csv.clear()  # 清空緩衝區

        frame_count += 1
        time.sleep(1)

# 啟動儲存影像和數據的執行緒
threading.Thread(target=save_images_and_data, daemon=True).start()
...
def update_plot(frame):
    ...
    # 如果正在錄製，則將影像和數據放入緩衝區
    if recording:
        image_buffer["left_eye"].append(left_eye_img)
        image_buffer["right_eye"].append(right_eye_img)
        image_buffer["scene"].append(scene_img)

        # 準備數據寫入緩衝區
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        data_buffer_csv.append([timestamp] + data_buffer["x"][-1:] + data_buffer["y"][-1:] + data_buffer["z"][-1:])
